...
export function ProjectsPage({ onNavigate }: ProjectsPageProps) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <form className="liquid-glass rounded-2xl p-6 flex flex-col gap-4" onSubmit={handleCreateProject}>
          <h2 className="text-sm font-bold text-zinc-200 uppercase tracking-widest flex items-center gap-2">
            <i className="ph-bold ph-folder-plus text-zinc-400" /> Create Project
          </h2>
          ...
          <button
            type="submit"
            disabled={saving}
            className="mt-2 px-4 py-2.5 bg-accent-500 hover:bg-accent-400 disabled:opacity-60 text-zinc-950 border border-transparent rounded-lg text-sm font-bold shadow-[0_0_15px_rgba(245,158,11,0.2)] transition-all flex justify-center items-center gap-2"
          >
        ...
        <div className="md:col-span-2 liquid-glass rounded-2xl p-8 flex flex-col justify-center relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">
            <i className="ph-bold ph-info text-accent-500 mr-1" /> Project Board
          </p>
          <h2 className="text-2xl font-bold text-white tracking-tight mb-2">Project lifecycle preserved</h2>
      ...
      {loading ? (
        <div className="liquid-glass rounded-2xl p-8 text-sm text-zinc-400">Loading project board...</div>
      ) : data ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div key={status} className="liquid-glass rounded-2xl p-5 flex flex-col h-[700px]">
                <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
                  <h2 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2">
                    <i className={`ph-bold ${statusIcons[status]} text-zinc-400`} /> {data.status_labels[status]}
                  </h2>
                  <span className="px-2 py-0.5 bg-black/40 border border-white/10 rounded text-[10px] font-mono text-zinc-400">
                    {projects.length}
                  </span>
                </div>
                <div className="flex-1 overflow-y-auto pr-2 space-y-3">
                  {projects.length ? (
                    projects.map((project) => (
                      <div
                        key={project.id}
                        className="bg-black/40 border border-white/10 rounded-xl p-4 group hover:border-accent-500/50 transition-colors flex flex-col gap-3"
                      >
                        <div>
                          <h3 className="text-sm font-bold text-white mb-1 group-hover:text-accent-400 transition-colors">
                            {project.name}
                          </h3>
                        ...
                        <div className="flex items-center justify-between border-t border-white/5 pt-3 mt-auto">
                          ...
                          <button 
                            className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white rounded text-[10px] font-semibold transition-colors border border-white/10" 
                    ...
                    <div className="text-center p-6 border border-dashed border-white/10 rounded-xl text-xs font-mono text-zinc-500">
                      No projects
                    </div>
                  )}
                </div>
